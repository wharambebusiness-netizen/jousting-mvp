...
```bash
npm test                                           # 4465 tests, 88 suites (all passing)
...
node operator/server.mjs --mcp                         # MCP tool server (stdin/stdout JSON-RPC)
```
...
operator/             Auto-continuation system (M2+M4+M5+P3+Phase1+Phase6+Phase15)
  ...
  claude-pool.mjs     Multi-terminal pool: multi-master (1-4), atomic task claiming, domain affinity, worktree integration, auto-handoff/dispatch/complete (Phase 15A+15E+17+19+20+21+22+26+66)
  ...
  master-coordinator.mjs  Multi-master heartbeat, domain affinity, dead master recovery (Phase 66)
  mcp-server.mjs      MCP tool server: 10 tools, 2 resources, JSON-RPC 2.0 stdin/stdout + HTTP proxy (Phase 66)
  ws.mjs              WebSocket event bridge (EventBus → clients, 108 bridged events) + binary WS for Claude terminals (Phase 15B+15E+19+20+22+66)
  ...
  routes/
    ...
    claude-terminals.mjs  Claude terminal REST API: list, spawn, resize, masters, pool-status, claim/release/complete-task (Phase 15C+15E+19+20+21+22+26+66)
    mcp.mjs           MCP HTTP proxy (POST /mcp) + tool definitions (GET /mcp/tools) (Phase 66)
    ...
    views.mjs         HTMX fragment routes for dashboard + git + missions + reports + settings + projects + console (M5+M6+P3+P9+P10+P66)
  ...
  coordination/       Inter-orchestrator coordination (Phase 6A+6B)
    ...
    worktree-manager.mjs Per-worker git worktree isolation: create, remove, merge, dry-run, createForMaster, mergeIfClean (Phase 66)
    persistent-queue.mjs Disk-backed task queue wrapper: atomic writes, crash recovery, in-flight reset (Phase 7b)
  __tests__/          1556 tests (registry, errors, server, views, file-watcher, process-pool, skills, skills-5b, coordination, coordination-integration, claude-terminals, claude-pool, settings, ws, orchestrator-worker, shared-memory, terminal-messages, swarm, swarm-resilience, swarm-routing, multi-master, multi-master-worktrees, master-coordinator, multi-master-console, mcp-server)
...
4465 tests across 88 suites. Engine: calculator (207), phase-resolution (66), gigling-gear (48), player-gear (46), match (100), playtest (128), gear-variants (225), ai (95). Orchestrator: dag-scheduler (59), mission-validator (64), cost-tracker (27), handoff-parser (26), agent-tracking (26), observability (28), mock-runner (26), test-filter (21), backlog-system (18), checkpoint (10), dry-run-integration (6), continuation (37), model-routing (13), role-registry (67). Operator: registry (21), errors (43), server (217), views (171), file-watcher (16), process-pool (65), skills (158), skills-5b (75), coordination (303), coordination-integration (22), claude-terminals (41), claude-pool (172), settings (37), ws (40), orchestrator-worker (23), shared-memory (74), terminal-messages (84), swarm (28), swarm-resilience (27), swarm-routing (25), master-console (21), multi-master (33), multi-master-worktrees (16), master-coordinator (24), multi-master-console (16), mcp-server (33), security (40), webhook-plugins (36), settings-webhooks-secrets (17), settings-backup-retention (17), multi-project (32), e2e (10), activity-stream (19). Run `npm test` to verify.
