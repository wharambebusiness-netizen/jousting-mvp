...
export function createWebSocketHandler({ server, events, claudePool, auth, pingIntervalMs, pongTimeoutMs, replayBufferSize } = {}) {
  ...
  const BRIDGED_EVENTS = [
    ...
    'claude-terminal:swarm-halted',
    'claude-terminal:task-recovered', 'claude-terminal:task-recovery-failed',
    ...
    'observer:started', 'observer:stopped', 'observer:action',
    'observer:circuit-open', 'observer:circuit-closed',
    'observer:budget-halt', 'observer:budget-warning',
  ];
